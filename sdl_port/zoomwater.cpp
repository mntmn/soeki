...
{ 
	...
	void run(int mx,int my);
		...
		void AlphasWater::init() 
		{
				...
				fxw = appleth; 
				fxh = appletw; 
				array = (uint32_t*)malloc(fxw * fxh * 4); 
				int counter=0; 
				int arrayptr=0; 
				for(int x=0;x<fxw;x++) 
				   for(int y=0;y<fxh;y++) 
						   array[arrayptr++]=0xff000000; 
				array_mt = (uint32_t*)malloc(fxw * fxh *4); 
 
				for(int x=0;x<65536;x++) 
				  array_mt[x]=0xff000000;  //leeres Feld zum Screenlöschen erstellen 
 
				water0 = (uint32_t*)malloc(fxw * fxh * 4); 
				water1 = (uint32_t*)malloc(fxw * fxh * 4); 
				phong = (uint32_t*)malloc(fxw * fxh *4);
				HMap=(uint32_t*)malloc(65536*4);
 
				sintab = (int32_t*)malloc(4*16384); 
				for(int y=0;y<16384;y++) 
				  sintab[y]=(int32_t)(sin(y*3.141592653589793238/90)*16); 
