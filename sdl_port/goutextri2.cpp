...
void AlphasMagic::MakeTable()
{
        ...
        for(int x=0;x<128;x++) //No brightness conversion to 0 to 254 anymore, 
          ...
          for(int y=0;y<256;y++)
          {
            int32_t dif=((long)256-(int32_t)y)/(long)127;
            int erg=(int)((int32_t)y+dif*(int32_t)x);
            if(erg>255)erg=255;
            if(erg<0)erg=0;
            multab2[128+x][y]=erg;
          }
...
void AlphasMagic::TriTexGouLine(int16_t z,int32_t x1, int32_t x2, int32_t y, int32_t tx1, int32_t tx2, int32_t ty1, int32_t ty2, int32_t g1, int32_t g2)
{
...
//End of clipping and calculation of screen start address
        uint32_t* arrayptr=array+(y*SCREEN_WIDTH)+x1;
		int16_t* ztab=Zmap+y*SCREEN_WIDTH+x1;
...
//Recalculate X-Delta because of clipping
        ...
        for(int x=0;x<=xd;x++)
        {
            ...
            if (array_tex1) pix=array_tex1[(txi>>16)+((tyi>>8)&0xff00)]; // FIXME ELSE WARN
...
void AlphasMagic::TriTexGouLine2(int32_t x1, int32_t x2, int32_t y, int32_t tx1, int32_t tx2, int32_t ty1, int32_t ty2, int32_t g1, int32_t g2)
{
		// WOW! Transparency ;)
...
void AlphasMagic::TriTexLine(int32_t x1, int32_t x2, int32_t y, int32_t tx1, int32_t tx2, int32_t ty1, int32_t ty2 )
{
		// Round to ensure that problems caused by rounding errors don't occur (jumping lines)
        x2&=0xFFFF0000;
        x1&=0xFFFF0000;
...
void AlphasMagic::ClearLine(int32_t z,int32_t x1, int32_t x2, int32_t y)
{
      x2&=0xFFFF0000;
      x1&=0xFFFF0000;
...
/*
 Coordinates are passed on in the 3 arrays (int32_t)
 [0]=x
 [1]=y
 [2]=z
 [3]=texture_x 0-256
...
void AlphasMagic::Triangle(TriangleDef *d,int16_t z)
{
	int32_t *dataa, *datab, *datac;

    int32_t xs1,xs2,xs3,txs1,txs2,txs3,tys1,tys2,tys3,gs1,gs2,gs3;
    int32_t* tempdata;
  ...
  int32_t gd1=datab[4]-dataa[4];
  int32_t gd2=datac[4]-dataa[4];
  int32_t gd3=datac[4]-datab[4];
  ...
  */
  ...
  for(int sz=dataa[1];sz<=datab[1];sz++)
  {
	...
	if(sz>=0)
	{
		if (z<500)
		{
			TriTexGouLine(z,xw1,xw2,(int32_t)sz,txw1,txw2,tyw1,tyw2,gw1,gw2);
		}
		else
		{
			ClearLine(z-600,xw1,xw2,(int32_t)sz);
		};
	}
	xw1+=xs1;
	xw2+=xs2;
...
void AlphasMagic::Transangle(TriangleDef *d )
{
  ...
  */
  ...
  for(int sz=dataa[1];sz<=datab[1];sz++)
  {
	...
	if(sz>=0)
	{
		TriTexGouLine2(xw1,xw2,(int32_t)sz,txw1,txw2,tyw1,tyw2,gw1,gw2);
	}
	xw1+=xs1;
	xw2+=xs2;
	txw1+=txs1;
...
void AlphasMagic::FlatTriangle( TriangleDef *d )
{
  ...
  */
  ...
  for(int sz=dataa[1];sz<=datab[1];sz++)
  {
	...
	if((sz>=0))
	{
		TriTexLine(xw1,xw2,(int32_t)sz,txw1,txw2,tyw1,tyw2);
	}
	xw1+=xs1;
	xw2+=xs2;
	txw1+=txs1;
