...
int32_t server_handler(void* p)
{
    ...
    // FIXME
		//snooze(5000);
		if (0) //net->LocalPlayer.connection->IsDataPending())
		{
			...
			{
				NW_Unit* newunit=(NW_Unit*)malloc(sizeof (NW_Unit));
				memcpy(newunit,(char*)content,sizeof (NW_Unit));
			...
			if (!strcmp(token,"upu0"))
			{
				NW_Unit* newunit=(NW_Unit*)malloc(sizeof (NW_Unit));
				uint16_t num=0;
				memcpy(&num,(char*)content,2);
				memcpy(newunit,(char*)content+2,sizeof (NW_Unit));
				memcpy(&nNW->DRUnits[num],newunit,sizeof (NW_Unit));
				free(newunit);
			};
...
void Store_NET::SendPacket(char* token,char* content,uint16_t len)
{
	uint16_t myid=LocalPlayer.id;
...
bool Store_NET::UpdateUnit(uint16_t num,NW_Unit* newunit,int slice)
{
	if (!slice)
	{
		char* content=(char*)malloc(2+sizeof(NW_Unit));
	...
	else
	{
		...
		char* content=(char*)malloc(2+sizeof(NW_USlice_1));
		memcpy(content,&num,2);
