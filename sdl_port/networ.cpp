...
//#include <InterfaceKit.h>
	...
	Store_ZON ZON;
	#include "platform/sdl/assd.h"
	assd ASS;
	#include "independent/gfx32.h"
	gfx_32 G32(640,480);
  #include "platform/sdl/storage.h"
	Store_DAT DAT;
  #include "platform/sdl/textures.h"
	Store_TEX TEX;
...
void snooze(int ms) {
  //printf("snooze %d\n",ms);
    ...
    //		snooze(100000);
		...
		G32.distort=(uint8_t*)malloc(256*300);
	...
	int32_t Flipper(void* p)
	{
		...
		while (universe->running)
		{
			...
			uint32_t* chunk=universe->Chapes[num].chunk;
			
			int x=universe->MouseX();
			int y=universe->MouseY();

			uint32_t* work=universe->NWWindow->kick_me;
	...
	void SoekiApp::main_thread()
	{
		...
		// Gogogo!
		G32.Use(NW.Chunky);
		for (uint32_t fill=0; fill<NW_WIDTH*NW_HEIGHT; fill++) *(NW.Chunky+fill)=0;
    ...
    // FIXME
      ...
      selection = 1;
		...
		//TEX.LoadRawTexture(0,"grass2-stone-br.dat",TEX_TOPLEFT|TEX_TOPRIGHT|TEX_BOTTOMLEFT|TEX_BOTTOMRIGHT);
		TEX.LoadTexture(0,"desert.png",TEX_TOPLEFT|TEX_TOPRIGHT|TEX_BOTTOMLEFT|TEX_BOTTOMRIGHT);
		...
		*/
		...
		TEX.LoadTexture(15,"grass2.png",TEX_TOPLEFT|TEX_TOPRIGHT|TEX_BOTTOMLEFT|TEX_BOTTOMRIGHT);
		...
		TEX.LoadTexture(30,"stone.png",TEX_TOPLEFT|TEX_TOPRIGHT|TEX_BOTTOMLEFT|TEX_BOTTOMRIGHT);
		//TEX.LoadRawTexture(30,"grass2-stone-br.dat",TEX_TOPLEFT|TEX_TOPRIGHT|TEX_BOTTOMLEFT|TEX_BOTTOMRIGHT);
		...
		for (x=11;x<15;x++) for (int y=11;y<15;y++) NW.Tex[x][y]=15;
...
//						NW.LandQuad2(gridlockx+NW.mapx+x,gridlocky+NW.mapy+y,i);
	...
	int main(int argc,char** argv)
	{
		...
		const char* pathbuf = argv[0];
		
		NW.SOEKI_PATH=(char*)malloc(strlen(pathbuf)+1);
		strcpy(NW.SOEKI_PATH,pathbuf);
		
		printf ("Application Path: %s\n",NW.SOEKI_PATH);

    SoekiApp* app = new SoekiApp();
    app->ReadyToRun();
