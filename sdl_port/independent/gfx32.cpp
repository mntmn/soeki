...
void gfx_32::Use(uint32_t* what)
{
  work=what;
};

void gfx_32::Line(int x1, int y1, int x2, int y2, uint32_t colour)
{
  uint32_t* dest=work;
  int xDiff,yDiff,
		xDirection,
...
void gfx_32::Grab(int num,int x,int y,int w,int h)
{
  ...
  uint32_t* chunk=(uint32_t*)malloc((w+1)*(h+1)*4);
  ...
  uint32_t* peek=work+(y*NW_WIDTH)+x;
...
void gfx_32::Grap(uint16_t* src,int num,int x,int y,int w,int h)
{
  if (!Chapes[num].chunk) Chapes[num].chunk=(uint32_t*)malloc((w+1)*(h+1)*4);
  Chapes[num].w=w;
  Chapes[num].h=h;
  Chapes[num].xhandle=0;
  Chapes[num].yhandle=0;
		
  uint32_t* chunk=Chapes[num].chunk;
		
  int loop=0;
		
  uint16_t* peek=src+(y*NW_WIDTH)+x;
...
void gfx_32::Blit(int num,int x,int y)
{
  ...
  uint32_t* poke=work+(y*NW_WIDTH)+x;
...
void gfx_32::CBlit(int num,int x,int y)
{
  ...
  uint32_t* onewchunk=(uint32_t*)malloc(size*4);
  uint32_t* newchunk=onewchunk;
  uint32_t sz=0;
  while (sz<size)
		{
			uint32_t block=(*(chunk)&0xff000000)>>24;
			while (block-->0)
        {
...
void gfx_32::CZBlit(int num,int x,int y,int16_t* zbuf,int16_t z,int smx,int smy)
{
  uint32_t* chunk=Chapes[num].chunk;
  if (!chunk) return;
  x-=Chapes[num].xhandle;
  ...
  int16_t* zmap=zbuf+((y+smy+64)*BUFFER_WIDTH)+x+128+smx;
...
void gfx_32::ZBlit(int num,int x,int y,int16_t* zbuf,int16_t z,int smx,int smy)
{
  uint32_t* chunk=Chapes[num].chunk;
  if (!chunk) return;
  x-=Chapes[num].xhandle;
  ...
  int scrmod=(NW_WIDTH-w)-1;
  int zscrmod=(BUFFER_WIDTH-w)-1;
  int16_t* zmap=zbuf+((y+smy+32)*BUFFER_WIDTH)+x+64+smx;
...
void gfx_32::ZSBlit(int num,int x,int y,int16_t* zbuf,int16_t z)
{
  uint32_t* chunk=Chapes[num].chunk;
  if (!chunk) return;
  x-=Chapes[num].xhandle;
  ...
  int scrmod=(NW_WIDTH-w)-1;
  int16_t* zmap=zbuf+(y*NW_WIDTH)+x;
  ...
  while (h-->=0)
			...
			for (loop=w; loop>=0; loop--)
        {
          if (*(chunk)!=0xff0000 && *(zmap)<=z)
            {
              uint32_t pix=*(poke);
              int16_t r=((pix & 0xff0000)>>16)-10;
              int16_t g=((pix & 0xff00)>>8)-10;
              int16_t b=(pix & 0xff)-10;
              if (r<0) r=0;
              if (g<0) g=0;
              if (b<0) b=0;
              *(poke)=r<<16|g<<8|b;
...
void gfx_32::Blint(int num,int x,int y,uint8_t offs)
{
  uint32_t* chunk=Chapes[num].chunk;
  if (!chunk) return;
  x-=Chapes[num].xhandle;
...
void gfx_32::Plit(uint16_t* dest,int num,int x,int y)
{
  uint32_t* chunk=Chapes[num].chunk;
  x-=Chapes[num].xhandle;
  y-=Chapes[num].yhandle;
  ...
  uint16_t* poke=dest+(y*NW_WIDTH)+x;
  int scrmod=(NW_WIDTH-w)-1;
...
void gfx_32::Blit16(uint16_t* dest,int num,int x,int y)
{
  uint32_t* chunk=Chapes[num].chunk;
  x-=Chapes[num].xhandle;
  y-=Chapes[num].yhandle;
...
void gfx_32::Rect(int x,int y,int x1,int y1,uint32_t col)
{
  if (x>x1)
		{
			int temp=x;
  ...
  int d=y1-y;
 loop2:
  uint32_t* poke=work+(y+d)*NW_WIDTH+x;
  int c=x1-x;
 loop1: *(poke++)=col;
  if (c--) goto loop1;
  if (d--) goto loop2;
...
void gfx_32::Box(int x,int y,int x1,int y1,uint32_t col)
{
  if (x>x1)
		{
			int temp=x;
...
void gfx_32::Box(int x,int y,int x1,int y1,uint32_t col,uint32_t col2)
{
  if (x>x1)
		{
			int temp=x;
  ...
  for (int d=y; d<=y1; d++)
		{
			uint32_t* poke=work+d*NW_WIDTH+x;
...
void gfx_32::Kopy(uint32_t* src,uint32_t* dest)
{
...
void gfx_32::ScrollHoriz(uint32_t* src,uint32_t* dest,int16_t* zsrc,int16_t* zdst)
{
  /*uint32_t* die=0x00000000;
   *(die)=1;*/
  if (dest<src)
		{
			for (uint32_t lot=BUFFER_HEIGHT; lot>0; lot--)
        {
          /*for (uint32_t xlot=WIDTH; xlot>0; xlot--)	{*(dest++)=*(src++);};
            for (uint32_t xlot=WIDTH; xlot>0; xlot--) {*(zdst++)=*(zsrc++);};*/
          ...
          zsrc+=WIDTH+64;
          zdst+=WIDTH;
          for (uint32_t xlot=64; xlot>0; xlot--) {*(zdst++)=-999;};
        };
		}
  else
		{
			...
			for (uint32_t lot=BUFFER_HEIGHT; lot>0; lot--)
        {
          ...
          for (uint32_t xlot=64; xlot>0; xlot--) {*(zdst--)=-999;};
          for (uint32_t xlot=WIDTH; xlot>0; xlot--) {*(dest--)=*(src--); *(zdst--)=*(zsrc--);};
...
void gfx_32::ScrollVert(uint32_t* src,uint32_t* dest,int16_t* zsrc,int16_t* zdst)
{
  if (dest<src)
		{
			for (uint32_t lot=BUFFER_HEIGHT-32; lot>0; lot--)
        {
          for (uint32_t xlot=BUFFER_WIDTH; xlot>0; xlot--) {*(dest++)=*(src++); *(zdst++)=*(zsrc++)-2;};
        };
			for (uint32_t lot=BUFFER_WIDTH*32; lot>0; lot--) *(zdst++)=-999;
		}
  else
		{
...
void gfx_32::ScrollDiag(uint32_t* src,uint32_t* dest,int16_t* zsrc,int16_t* zdst)
{
  //uint32_t width=BUFFER_WIDTH-64;
  uint32_t xlot=0;
  uint32_t lot=0;
  if (dest<src)
		{
...
void gfx_32::SmoothCopy(uint32_t* base,int xoff,int yoff,uint32_t* dest)
{
