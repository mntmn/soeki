...
*/
	...
	MouseAround::MouseAround() : BView::BView(BRect(0,0,WIDTH-1,HEIGHT-1),"HECKLER",0,0)
	{
	};
	...
	bool NetworDisplay::md()
	{
		...
		uint32 really;
		Lock();
		m->GetMouse(&there,&really,false);
		Unlock();
		return really;
	};
	
	uint32 NetworDisplay::mx()
	{
		BPoint there;
		uint32 really;
		Lock();
		m->GetMouse(&there,&really,false);
		Unlock();
		return (uint32)there.x;
	};
	
	uint32 NetworDisplay::my()
	{
		BPoint there;
		uint32 really;
		Lock();
		m->GetMouse(&there,&really,false);
		Unlock();
		return (uint32)there.y;
	};
	
	NetworDisplay::NetworDisplay() : BWindowScreen::BWindowScreen("Heckler & Koch",B_32_BIT_640x480,&anyerror)
	{
		didthejob=false;
		if (anyerror != B_OK)
		{
			printf ("Failed to takeover 640x480 32 bits screen.\n");
			printf ("Are you sure that your graphics card supports this mode?\n");
			printf ("The error: %s",strerror(anyerror));
			exit(666);
		};
	...
	{
		if (!really) return;
		SetFrameBuffer(WIDTH,HEIGHT*2);

		cardguts=CardInfo();
		
		char debuffer[200];
		
		sprintf(debuffer,"Card ID      : 0x%x\n",cardguts->id);
		nASS->Debug(debuffer);
		sprintf(debuffer,"Card Version : 0x%x\n",cardguts->version);
		nASS->Debug(debuffer);
		sprintf(debuffer,"Buffer width : %d\n",cardguts->width);
		nASS->Debug(debuffer);
		sprintf(debuffer,"Buffer height: %d\n",cardguts->height);
		nASS->Debug(debuffer);
		sprintf(debuffer,"Color order  : %c %c %c %c\n",cardguts->rgba_order[0],cardguts->rgba_order[1],cardguts->rgba_order[2],cardguts->rgba_order[3]);
		nASS->Debug(debuffer);
		sprintf(debuffer,"Buffer at    : 0x%x\n",(unsigned int)cardguts->frame_buffer);
		nASS->Debug(debuffer);
		
		kick_me=(uint32*)cardguts->frame_buffer;
		HardwareBlit=(blit_hook)CardHookAt(7);
		HardwareSync=(sync_hook)CardHookAt(10);		
		printf ("HWBlit: %x\n",HardwareBlit);
		printf ("HWSync: %x\n",HardwareSync);
	...
	void NetworDisplay::MessageReceived(BMessage* msg)
	{
		/*char debuffer[100];
		...
		case B_KEY_DOWN:
			...
			len=CHARZ[KEYZ->normal_map[key]];
			if (len==1)
			{
				kd=CHARZ[KEYZ->normal_map[key]+1];
				//printf ("key: %x\n",kd);
			};
			break;
		//case B_KEY_UP:
...
//*****************************************************************************
	...
	void Store::Show()
	{
		...
		if (GetKey()==0x21)
		{
			...
			Dat* TheFile=nDAT->Open(Out,DAT_WRITE);
	...
	void Store::LoadChape(int chapenum,char fname[],int w,int h)
	{
		...
		Dat* TheFile=nDAT->Open(name,DAT_READ);
...
/*		if (Console_Running)
		...
		if (!res)
		{
			printf("Error loading chape %s",name);
//			exit(1005);
		};
	};
	
	void Store::LoadChape(int chapenum,char fname[])
	{
		char* name=fname;
		int res=0;
...
//			exit(1005);
	...
	void Store::LoadChapes(int chapenum,int max,char fname[])
	{
		char* name=fname;
		int res=0;
		char buf[5]="\0\0\0\0";
	...
	ReadLoop:
		...
		else
		{
			...
			ncnt++;
			Chapes[chapenum].chunk=(uint32*)malloc(ncnt*4);
...
outback:	nDAT->Close (TheFile);
	...
	void Store::LoadBuilding(int num,char fname[])
	{
		FILE* test=fopen("/boot/home/test.amxb","r");
		char header[5];
		header[4]=0;
		...
		for (int i=0; i<4; i++)
		{
			...
			if (!strcmp(header,"STAT"))
			{
				...
				Chapes[chp].chunk=(uint32*)malloc((Chapes[chp].w+1)*(Chapes[chp].h+1)*4);
	...
	Store::Store()
	{
		...
		Chunky2=(uint32*)malloc(((BUF_WIDTH)*(BUF_HEIGHT))*4);
	...
	int Store::SuperDynt(int x,int y,char* text)
	{
		int i=0;
		int base=200;
		int c=0;
	...
	void Store::NewStream(int chp,char fname[])
	{
		char Buf[5]="    ";
		if (Stream)
		{
